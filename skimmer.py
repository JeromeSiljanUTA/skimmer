...
def cleanup(customcash_path, altitude_path, cashplus_path, discover_path, wf_path):
    ...
    # Wells Fargo Cleanup
    wf = pd.read_csv(wf_path, header=None, usecols=[0, 1, 4]).rename(
        columns={0: "Date", 4: "Name", 1: "Amount"}
    )
...
def add_tags(tagless):
    ...
    for entry in tagless:
        ...
        if mine == "yes" or mine == "y":
            ...
            cursor.execute(
                "INSERT INTO info(ID, Merchant, Notes, Tags) VALUES("
                + entry["ID"]
                + ', "'
                + merchant
                + '", "'
            ...
            cursor.execute(
                'INSERT INTO info(ID, Tags) VALUES("' + entry["ID"] + '", "Mom")'
            )
...
def insert_new(paths):
    ...
    for row in all_rows:
        ...
        try:
            cursor.execute(
                "INSERT INTO main(Date, Name, Amount, Category, Card) VALUES ("
                + date
                + name
                + amount
                + category
